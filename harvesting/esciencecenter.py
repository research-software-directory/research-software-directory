...
def get_project_urls(soup):
    ...
    for project in soup.findAll("article"):
        ...
        for a in project.findAll("a"):
            url = a.get("href")
            if is_project_url(url): project_url = url
        project_field,project_status = get_project_field_and_status(project)
        if project_url != "":
            project_urls.append({"url":project_url,"field":project_field,"status":project_status})
    return(project_urls)
...
def get_project_data(project_urls):
    ...
    for project_url in project_urls:
        counter += 1
        print(f'fetching project page {counter}/{len(project_urls)} {project_url["url"]}')
        ...
        if request_was_successful(response_project):
            ...
            project["imageUrl"] = get_project_image_url(soup)
            ...
            logger.warning(f"failed fetching project page {counter} {project_url}")
        time.sleep(1)
    return(project_data)
...
def get_data_from_rsd(token,directory):
    ...
    api_response = requests.get(api_url,headers={'Authorization': 'Bearer %s' % token},verify=False)
    if not request_was_successful(api_response):
        api_data = {}
    else:
        api_soup = BeautifulSoup(api_response.text, 'html.parser')
        api_data = json.loads(remove_html_links(str(api_soup)))
    return(api_data)


def get_data_by_title(api_data,title):
    for data in api_data[::-1]:
        if data["title"] == title: return(dict(data))
    return({})


def get_data_by_name(api_data,name):
    for data in api_data[::-1]:
        if data["name"] == name: return(dict(data))
    return({})
...
def make_person_name(person_data):
    ...
    if "nameParticle" in person_data and person_data["nameParticle"] is not None and person_data["nameParticle"] != "":
        if name != "": name += " "
        name += person_data["nameParticle"]
    if person_data["familyNames"] != "":
        if name != "": name += " "
        name += person_data["familyNames"]
    if "nameSuffix" in person_data and person_data["nameSuffix"] is not None and person_data["nameSuffix"] != "":
        if name != "": name += " "
...
def split_name(name):
    ...
    if len(name_part_list) > 0: given_names = name_part_list.pop(0)
    else: given_names = ""
    while len(name_part_list) > 0 and len(name_part_list[0]) == 1 or name_part_list[0][-1] == ".":
        given_names += " "+name_part_list.pop(0)
    name_particle = ""
    while len(name_part_list) > 0 and re.search("^(d|d'|de|der|het|t|'t|uit|uyt|van|vande|vander)$",name_part_list[0],flags=re.IGNORECASE):
        if name_particle == "": name_particle = name_part_list.pop(0)
        else: name_particle += " "+name_part_list.pop(0)
...
def make_new_person_data(name):
    ...
    new_person_data["givenNames"],new_person_data["nameParticle"],new_person_data["familyNames"] = split_name(name)
...
def add_new_person_to_database(token,api_persons_data,name):
    ...
    person_api_response = requests.post(person_api_url,json=new_person_data,headers={'Authorization': 'Bearer %s' % token},verify=False)
...
def get_suffix(url):
    ...
    if len(url_parts) > 0: return(url_parts[-1])
    else: return("")


def make_new_organization_data(name_data):
    new_organization_data = {"primaryKey":{"id":str(uuid.uuid4()),"collection":"organization"}}
    ...
    new_organization_data["url"] = "http://fix.me"
    if "name" in name_data: new_organization_data["name"] = name_data["name"]
    img_data = ""
    img_suffix = "png"
    ...
    new_organization_data["logo"] = {"data":img_data, "mimeType":"image/"+img_suffix}
...
def add_new_organization_to_database(token,api_organizations_data,name_data):
    ...
    organization_api_response = requests.post(organization_api_url,json=new_organization_data,headers={'Authorization': 'Bearer %s' % token},verify=False)
    if request_was_successful(organization_api_response):
        ...
        logger.warning(f"failed storing organization in database: {name}")
        

# see required list and patterns in backend/schemas/project.json
def make_new_project_data(project_data_source):
    new_project_data = {}
    new_project_data["callUrl"] = "https://doi.org/fix.me"
    new_project_data["codeUrl"] = "https://github.com/fix.me"
    ...
    new_project_data["description"] = remove_html_links(project_data_source["text"])
    new_project_data["grantId"] = "FIX ME"
    new_project_data["imageCaption"] = "FIX ME FIX ME"
    new_project_data["imageUrl"] = project_data_source["imageUrl"]
...
def add_new_project_to_database(token,api_projects_data,project_data_source):
    ...
    project_api_response = requests.post(project_api_url,json=new_project_data,headers={'Authorization': 'Bearer %s' % token},verify=False)
    if request_was_successful(project_api_response):
        ...
        logger.warning(f'failed storing project in database: {project_api_response} {project_data_source["title"]}')


def fix_role(role):
    role = re.sub("Principal Investigator","Principal investigator",role,flags=re.IGNORECASE)
    role = re.sub("eScience Senior Research Engineer","Senior eScience Research Engineer",role,flags=re.IGNORECASE)
    role = re.sub("Project Coordinator","eScience Coordinator",role,flags=re.IGNORECASE)
...
def add_team_member(team_member_list,updated_team_member):
    ...
    for team_member in team_member_list:
        if team_member["foreignKey"]["id"] == updated_team_member["foreignKey"]["id"]:
            ...
            update = True
    if not update: team_member_list.append(updated_team_member)


def guess_affiliation(team_member,related_organizations):
    if re.search("(escience|lead|director|manager|phd)",team_member["role"],flags=re.IGNORECASE):
        team_member["affiliations"] = [{"foreignKey":{"id":"nlesc","collection":"organization"}}]
    elif re.search("principal investigator",team_member["role"],flags=re.IGNORECASE) and len(related_organizations) == 1:
...
def get_source_team(token,project_data_source,api_persons_data,related_organizations):
    acceptable_roles = ["Principal investigator", "Contact person", "eScience Research Engineer", "Senior eScience Research Engineer",
                        "eScience Coordinator", "Technical Lead", "PhD student", "Community Manager", "Director of Operations",
                        "Director of Technology"]

    team = []
    for team_member_data_source in project_data_source["team"]:
        team_member_data_target = find_team_member(api_persons_data,team_member_data_source["name"])
        if len(team_member_data_target) == 0:
            team_member_data_target = add_new_person_to_database(token,api_persons_data,team_member_data_source["name"])
        ...
        if "role" in team_member_data_source and team_member_data_source["role"] != "":
            team_member_data_update["role"] = fix_role(team_member_data_source["role"].strip())
            if not team_member_data_update["role"] in acceptable_roles:
                logger.warning(f'unknown role: {team_member_data_update["role"]}')
        else:
            team_member_data_update["role"] = "eScience Research Engineer"
        team_member_data_update = guess_affiliation(team_member_data_update,related_organizations)
        add_team_member(team,team_member_data_update)
    return(team)


def get_source_related_organizations(token,project_data_source,api_organizations_data):
    related_organizations = []
    for related_organization_name in project_data_source["partners"]:
        related_organization = get_data_by_name(api_organizations_data,related_organization_name["name"])
        if not "primaryKey" in related_organization or ("logo" in related_organization and related_organization["logo"]["data"] == ""):
            related_organization = add_new_organization_to_database(token,api_organizations_data,related_organization_name)
...
def get_source_related_projects(project_data_source,api_projects_data):
    ...
    for related_project_title in project_data_source["related_project_data"]:
        ...
        if not "primaryKey" in related_project:
...
def update_project_data(token,api_projects_data,api_organizations_data,api_persons_data,project_data_source,project_data_target):
    project_data_target["description"] = remove_html_links(project_data_source["text"])
    project_data_target["imageUrl"] = project_data_source["imageUrl"]
    project_data_target["updatedBy"] = "esciencecenter.nl.crawler"
    project_data_target["updatedAt"] = get_current_time_string()
    project_data_target["related"]["organizations"] = get_source_related_organizations(token,project_data_source,api_organizations_data)
    project_data_target["related"]["projects"] = get_source_related_projects(project_data_source,api_projects_data)
    project_data_target["team"] = get_source_team(token,project_data_source,api_persons_data,project_data_target["related"]["organizations"])
    return(project_data_target)


def update_project_in_database(token,api_projects_data,api_organizations_data,api_persons_data,project_data_source,project_data_target):
    project_data_target = update_project_data(token,api_projects_data,api_organizations_data,api_persons_data,project_data_source,project_data_target)
    project_api_url = "https://localhost/api/project/"+project_data_target["primaryKey"]["id"]
    project_api_response = requests.put(project_api_url,json=project_data_target,headers={'Authorization': 'Bearer %s' % token},verify=False)
    if request_was_successful(project_api_response):
        print(f'updated project "{project_data_source["title"]}"')
        return(True)
    else:
        logger.warning(f'failed updating project in database: {project_api_response} {project_data_source["title"]}')
        return(False)
...
def main(argv):
    ...
    for project_data_source in project_data_website:
        project_data_target = find_project_in_database(token,api_projects_data,project_data_source)
        success = update_project_in_database(token,api_projects_data,api_organizations_data,api_persons_data,project_data_source,project_data_target)
        update_project = True
        if not success: failed_project_updates += 1
